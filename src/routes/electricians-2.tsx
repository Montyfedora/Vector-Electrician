...
function Step5Account({
  ...
  const genOutline = useServerFn(generateOutline);
  ...
  const [progress, setProgress] = useState({ label: "", current: 0, total: 1 });
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      // Save the lead first so we never lose it, even if generation fails.
      ...
      // Step 1: outline (fast).
      setProgress({ label: "Planning your article…", current: 0, total: 1 });
      const outlineRes = await genOutline({ data: genArgs });
      if (!outlineRes.ok) {
        setErr(outlineRes.error);
        setPhase("done");
        setBusy(false);
        return;
      }
      ...
      for (let i = 0; i < sections.length; i++) {
        ...
        const sec = await genSection({
          data: { ...genArgs, title: outline.title, h2: sections[i] },
        });
        if (sec.ok) {
          md += sec.markdown + "\n\n";
          setMarkdown(md);
      ...
      // Step 3: FAQs.
      setProgress({ label: "Adding FAQs people ask…", current: sections.length, total });
      const faqRes = await genFaqs({ data: genArgs });
      if (faqRes.ok) {
        md += faqRes.markdown + "\n\n";
        setMarkdown(md);
      }
      ...
      // Persist + email the finished article (no extra Claude call).
      try {
        const saved = await saveAndEmail({
          data: {
            leadId,
            markdown: md,
            toEmail: data.email,
  ...
  if (phase === "writing" || (phase === "done" && markdown))
    return (
      <LiveArticleView
        markdown={markdown}
        brand={data.brand}
    ...
    return (
      <div className="rounded-2xl border border-emerald-500/30 bg-emerald-500/5 p-8 text-center">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          Thanks{data.brand ? `, ${data.brand}` : ""} — your details are saved. We'll follow up at{" "}
          <span className="font-semibold text-foreground">{data.email}</span> shortly.
        </p>
        {err && <p className="mt-3 text-xs text-muted-foreground">{err}</p>}
      </div>
    );
...
function MarkdownRender({ markdown }: { markdown: string }) {
  ...
  for (const raw of lines) {
    ...
    if (/^###\s+/.test(line)) {
      ...
      flushList();
      blocks.push(
        <p key={key++} className="leading-relaxed text-foreground/90 mt-2">
          {renderInline(line)}
        </p>,
      );
    }
...
/* ────────────────────────────────────────────────────────────────────────── */
/*  Live article view — reveals the article as it's written                  */
/* ────────────────────────────────────────────────────────────────────────── */

function LiveArticleView({
  markdown,
  brand,
  domain,
  email,
    ...
    <article className="rounded-2xl border border-border bg-card overflow-hidden">
      ...
      {writing && (
        <div className="border-b border-border bg-gradient-to-r from-primary/10 to-accent/10 px-6 py-4">
          <div className="flex items-center gap-3">
            <div className="h-8 w-8 rounded-full bg-gradient-to-br from-primary to-accent grid place-items-center text-primary-foreground animate-pulse">
              ✨
            </div>
            <div className="flex-1">
              <p className="text-sm font-semibold">
                Vector AI is writing your article for {brand}…
              </p>
...
function PublishPanel({
  ...
  const articleTitle = titleFromMd(markdown);
  const articleHtml = mdToHtml(markdown);
  ...
  const downloadHtml = () => {
    const html = `<!doctype html><html><head><meta charset="utf-8"><title>${articleTitle}</title></head><body>${articleHtml}</body></html>`;
